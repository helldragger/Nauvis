...
async def fireMessageEvent(event_type:EventType, msg_id:int, event_data:object):
    try:
        ...
        pass
    print("INFO>"+event_type.name+" message event triggered on msg#"+str(msg_id))
...
def registerScreen(ref, *types:EventType) :
    ...
    for T in types:
        if T not in msgWatcherRegistry.keys():
            msgWatcherRegistry.setdefault(T, {id: {ref}})
